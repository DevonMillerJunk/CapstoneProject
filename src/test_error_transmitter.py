...
# Call with test_transmitter {addr} {test_time} {num_errors}
def run_test(arguments):
    arg_addr = int(arguments[1])
    test_duration = float(arguments[2])
    num_errors = int(arguments[3])
    ...
    try:
        ...
        while time.time() - start_t < test_duration:
            print("calling test_send_with_errors")
            node.test_send_with_errors(message, conn_addr, num_errors)
        ...
        print(f'True (Includes Header Data) Data Rate: Sent {8 * node.sent_bytes} bits in {end_t - start_t} seconds. {float(8 * node.sent_bytes) / (end_t - start_t)}bps')
